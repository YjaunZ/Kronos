...
def plot_candlestick_with_ma_macd_and_prediction_continuous_long(historical_df, prediction_df, stock_symbol,
                                                                 prediction_days=60, candle_width=0.6):
    """
    绘制长线预测带MA20、MA60、MACD和预测的K线图，消除假期导致的时间间隔，使数据连续显示
    参数:
    ...
    # 设置标题和标签
    ax1.set_title(
        f'{stock_symbol} - Long-term Prediction K-line Chart with MA & MACD (Continuous Time)\nPrediction Period: {prediction_days} days',
    ...
    step = max(1, total_points // 20)  # 为长线预测增加更多标签
...
def generate_long_term_prediction_kline_with_ma_macd_continuous(stock_symbol, prediction_days=60, candle_width=0.6):
    """
    生成带MA20、MA60、MACD和成交量的长线预测K线图的主要函数（连续时间轴版本）
    参数:
    - candle_width: K线宽度，控制K线之间的距离 (0.1-1.0)
    """
    print(f"开始生成股票 {stock_symbol} 的带MA/MACD的长线预测K线图（连续时间轴）...")
    print(f"K线宽度设置为: {candle_width}")
    print(f"长线预测天数: {prediction_days} 天")
    ...
    print("正在获取股票数据...")
    try:
        df = get_stock_data(stock_symbol, days=400)  # 获取近400天数据用于长线预测
        if df.empty:
            print("获取的股票数据为空")
            return
    ...
    print("正在生成带MA/MACD的连续时间轴长线预测K线图...")
...
if __name__ == "__main__":
    ...
    # 示例：生成带MA/MACD的长线预测K线图（连续时间轴）
    stock_symbol = '601012'  # 可以修改为目标股票代码
    prediction_days = 60  # 长线预测天数
    candle_width = 0.6  # K线宽度，控制K线之间的距离 (0.1-1.0)

    generate_long_term_prediction_kline_with_ma_macd_continuous(stock_symbol, prediction_days, candle_width)
