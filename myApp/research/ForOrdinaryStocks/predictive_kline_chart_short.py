...
def generate_short_term_prediction_kline_with_ma_macd_continuous(stock_symbol, prediction_days=10, candle_width=0.6):
    """
    生成带ma5、ma14、MACD和成交量的短期预测K线图的主要函数（连续时间轴版本）
    参数:
    ...
    # 4. 绘制K线图（包含ma5、ma14、MACD、成交量，使用连续时间轴）
...
def plot_candlestick_with_ma_macd_and_prediction_continuous_short(historical_df, prediction_df, stock_symbol,
    ...
    """
    绘制短期预测带ma5、ma14、MACD和预测的K线图，消除假期导致的时间间隔，使数据连续显示
    参数:
    - candle_width: K线宽度，控制K线之间的距离 (0.1-1.0)
    """
    # 合并历史和预测数据
    ...
    historical_df_extended['ma5'] = calculate_ma(historical_df_extended, 20)
    historical_df_extended['ma14'] = calculate_ma(historical_df_extended, 60)
    ...
    all_data['ma5'] = calculate_ma(all_data, 20)
    all_data['ma14'] = calculate_ma(all_data, 60)
    ...
    # 绘制ma5和ma14线
    ma5_values = combined_df_reset['ma5']
    ma14_values = combined_df_reset['ma14']
    indices = combined_df_reset['continuous_index']

    ax1.plot(indices, ma5_values, color='orange', linewidth=1, label='ma5')
    ax1.plot(indices, ma14_values, color='purple', linewidth=1, label='ma14')
    ...
    ma5_line = mlines.Line2D([], [], color='orange', linewidth=1, label='ma5')
    ma14_line = mlines.Line2D([], [], color='purple', linewidth=1, label='ma14')

    ax1.legend(handles=[historical_patch, predicted_patch, split_line, ma5_line, ma14_line],
...
if __name__ == "__main__":
    ...
    stock_symbol = 'BK0428'  # 可以修改为目标股票代码或板块代码如 'BK1033'
